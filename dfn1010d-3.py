...
def smd(pos, length, width, orientation, name):
    return "Layer 1;\n SMD %smm %smm -%s R%f '%s' (%fmm %fmm);" % (length, width, pad_roundness, orientation, name, pos[0], pos[1])
...
def create_footprint():
    ...
    res.append(smd((-0.75/2, 0.5+0.4/2), 0.35, 0.4, 0, 1))
    res.append(smd((0.75/2, 0.5+0.4/2), 0.35, 0.4, 0, 2))
    res.append(smd((0, -0.5-0.4/2), 0.4, 0.4,  0, 3))
    return '\n'.join(res) + '\n' + '\n'.join(wire)
