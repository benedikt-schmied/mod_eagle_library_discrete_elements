...
def smd(pos, length, width, orientation, name, flags):
    return "Layer 1;\n SMD %smm %smm -%s R%f %s '%s' (%fmm %fmm);" % (length, width, pad_roundness, orientation, flags, name, pos[0], pos[1])

def rect(pos, length, width, layer, name):
    return "%s ;\n RECT %s (%fmm %fmm)(%fmm %fmm);" % (layer, name, length, width, pos[0], pos[1])
...
def create_footprint():
    ...
    res.append(smd((-3.5/2 - 3.2/2, 1.6), 3.2, 3.2, 0, 1, "nostop nocream"))
    res.append(smd((3.5/2 + 3.2/2, 1.6), 3.2, 3.2, 0, 2, "nostop nocream"))
    res.append(smd((-3.5/2 -3.2/2, -0.3 - 0.8/2), 3.2, 0.8, 0, 3, "nostop nocream"))
    res.append(smd((3.5/2 + 3.2/2, -0.3 - 0.8/2), 3.2, 0.8, 0, 4, "nostop nocream"))
    res.append(rect((-3.8/2,0), -(3.8/2 + 1.2), 1.9, "Layer 29",  "R0"))
    res.append(rect((-3.8/2,0), -(3.8/2 + 1.2), 1.9, "Layer 31",  "R0"))
    res.append(rect((3.8/2,0), (3.8/2 + 1.2), 1.9, "Layer 29",  "R0"))
    res.append(rect((3.8/2,0), (3.8/2 + 1.2), 1.9, "Layer 31",  "R0"))
    res.append(rect((-3.8/2,-0.3), -(3.8/2 + 1.2), -0.3 - 0.8, "Layer 29",  "R0"))
    res.append(rect((-3.8/2,-0.3), -(3.8/2 + 1.2), -0.3 - 0.8, "Layer 31",  "R0"))
    res.append(rect((3.8/2,-0.3), (3.8/2 + 1.2), -0.3 - 0.8, "Layer 29",  "R0"))
    res.append(rect((3.8/2,-0.3), (3.8/2 + 1.2), -0.3 - 0.8, "Layer 31",  "R0"))
    return '\n'.join(res) + '\n' + '\n'.join(wire)
